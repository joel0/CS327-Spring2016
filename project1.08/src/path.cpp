...
int pathPopulate(dungeon_t* dungeonPtr, uint8_t** distGrid, uint8_t (*relDist)(dungeon_t*, int, int)) {
    ...
    for (int y = 0; y < HEIGHT; y++) {
        for (int x = 0; x < WIDTH; x++) {
            ...
            if (!isBorder(x, y)) {
                gridNodePtr = (gridNode_t *) malloc(sizeof(gridNode_t));
                gridNodePtr->x = x;
                gridNodePtr->y = y;
    ...
    while (heap.size) {
        gridNodePtr = (gridNode_t *) heap_remove_min(&heap);
...
gridNode_t* mallocGridNode(uint8_t* weightPtr, int x, int y) {
    gridNode_t* node = (gridNode_t *) malloc(sizeof(gridNode_t));
...
int32_t pathCmp(const void *keyPtr, const void *withPtr) {
    const gridNode_t* keyPtr2 = (const gridNode_t *) keyPtr;
    const gridNode_t* withPtr2 = (const gridNode_t *) withPtr;
